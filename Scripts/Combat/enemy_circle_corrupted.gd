...
func _physics_process(delta: float) -> void:
	...
	if(turn_ready):
		...
		if(target):
			...
			turn_angle = target.global_position - global_position.angle()+rng.randf_range(-2*PI/3,2*PI/3)
		else:
			if(global_position.distance_to(home_position) > 2000):
				# turns sharply towards home if far away
					global_rotation = lerp_angle(global_rotation,(home_position - global_position).angle()+rng.randf_range(-PI/6,PI/6),rotation_speed)
			else:
				# turns vaguely towards home 
					global_rotation = lerp_angle(global_rotation,(home_position - global_position).angle()+rng.randf_range(-PI/2,PI/2),rotation_speed)
		turn_cooldown()
	move_and_slide()
	
func fire():
	fire_ready = false
	fire_direction = fire_direction - PI + rng.randf_range(-PI/2,PI/2)
	AttackSpawner.spawn_bullets(global_position,fire_direction,"single",1,0,"default","circle","straight",1000,2,30,"circle","purple",0,0,0)
